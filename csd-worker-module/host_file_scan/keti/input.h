...
struct Snippet{
    ...
    Snippet(const char* json_, const char* file_path_){
        file_path = file_path_;//TEST
        ...
        total_block_count = 2444;
        // total_block_count = document["totalBlockCount"].GetInt();//추가
        ...
        int primary_count = document["pkNum"].GetInt();
        if(primary_count == 0){
          kNumInternalBytes = 8;
        }else{
          kNumInternalBytes = 0;
        ...
        //   if (*find(filtered_col.begin(), filtered_col.end(), (*iter).key_name) == (*iter).key_name) {
        //       need_pk = true;
        //       break;
        //   } 
        // }
